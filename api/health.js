...
module.exports=async function(req,res){
  ...
  return res.status(200).json({ok:true,service:'athena-ai',timestamp:new Date().toISOString(),chatProviders:providers(),providers:{
    gemini:geminiKeys().length>0,groq:Boolean(process.env.GROQ_API_KEY),openrouter:Boolean(process.env.OPENROUTER_API_KEY),search:Boolean(process.env.TAVILY_API_KEY),database:Boolean(process.env.MONGO_URI),
